...
public class Sala extends Joc {
	...
	public Sala() {  
		this(portaEntradaAleatoria()); // Llama al constructor con una puerta aleatoria
	}
	...
	// Método para agregar una puerta en la posición correcta (OPTIMIZADO)
	private void afegirPorta(String porta) {
		for (int i = 0; i < DIRECCIONS.length; i++) {
			if (DIRECCIONS[i].equals(porta) && portes[i].isEmpty()) {
				portes[i] = porta;
				numPortes++;
				break; // (NUEVO: evita iteraciones innecesarias)
			}
		}
	}
